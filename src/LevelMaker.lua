        ...
        if math.random(7) == 1 and x > 5 and x~= lockPosition and x < width - 3 then
            ...
            if x == keyPosition then
                table.insert(objects, GameObject {
                    ...
                    --for having different color of the key
            ...
            --in spawning a lock block
            elseif x == lockPosition then
                table.insert(objects, GameObject {
                    texture = 'keys-and-locks',
                    x = (x - 1) * TILE_SIZE,
                    y = (blockHeight - 1) * TILE_SIZE,
                    ...
                    -- to make it same color with the key
                    frame = 4 + keyandlockcolor,
                    collidable = true,
                    hit = false,
                    solid = true,

                    --in unlocking the lock brick
                    onCollide = function(obj)           
                        gSounds['powerup-reveal']:play()
                        for k, object in pairs(objects) do
                            if keyTaken and object == obj then
                                ...
                                GameObject {
                                    ...
                                    -- if you collide in the pole
                                    onCollide = function(obj)           
                                        gSounds['powerup-reveal']:play()
                                    end,
                                    
                                    --function if the player touches the pole
                                    onConsume = function(player, object)
                                        gStateMachine:change('play', {score = player.score, currentLevel = player.currentLevel})
                                    end
                                })
                                --in spawning his lover
                                table.insert(objects,
                                GameObject {
                                    texture = 'pink-alien',
                                    x = (x + 6) * TILE_SIZE - 11,
                    ...
                    --if we already unlocked the brick
                    onConsume = function(player, object)
